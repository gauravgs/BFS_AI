  ...
  this.random_split = function(container) {
    ...
    if (this.random(0, 1) == 0) {
      ...
      region1 = new BspContainer(
        container.x,
        container.y, // region1.x, region1.y
        this.random(1, container.w),
        container.h // region1.w, region1.h
      );
      region2 = new BspContainer(
        container.x + region1.w,
        container.y, // region2.x, region2.y
        container.w - region1.w,
        container.h // region2.w, region2.h
      );
      ...
      region1 = new BspContainer(
        ...
        container.y, // region1.x, region1.y
        container.w,
        this.random(1, container.h) // region1.w, region1.h
      );
      region2 = new BspContainer(
        container.x,
        container.y + region1.h, // region2.x, region2.y
        container.w,
        container.h - region1.h // region2.w, region2.h
      );
...
function BspTree(leaf) {
  ...
  this.getLeafs = function() {
    if (this.lchild === undefined && this.rchild === undefined)
      return [this.leaf];
    else return [].concat(this.lchild.getLeafs(), this.rchild.getLeafs());
  };
}
